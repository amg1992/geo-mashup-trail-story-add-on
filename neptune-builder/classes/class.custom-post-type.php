...
if( ! class_exists( 'NP_CPT' ) ) {
    ...
    class NP_CPT{
		...
		'show_ui'            => true,
		'show_in_menu'       => true,
        'taxonomies'         => array( 'post_tag', $taxonomies  ),
		'rewrite'            => true,
		...
		'supports'           => array( 'title', 'editor', 'author', 'thumbnail', 'excerpt', 'custom-fields', 'revisions', 'comments' ),
            ...
            if ($has_taxonomy === True) {
                ...
                register_taxonomy( 'itinerary-category', array($post_type), $args );
            }

            //$taxonomies = array($tax_args);
        ...
        public function register() {
            ...
            if ( (!empty($post_type) ) && (!empty($post_args) ) ) {
                register_post_type( $post_type, $post_args );
                flush_rewrite_rules();
            } else {
                echo 'Error registering CPT and flushing permalinks.';
            }
